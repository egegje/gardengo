...
(() => {
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(min(360px,100%),1fr));gap:clamp(36px,4.5vw,72px);align-items:center;">
      <div style="animation:ggFade .8s ease both;">
        ...
        <div class="hero-cta" style="display:flex;flex-wrap:wrap;gap:14px;margin-bottom:44px;">
          <a class="btn-primary btn-primary-lg hero-btn ed" data-path="hero.ctaPrimary" href="#contact" style="display:inline-flex;align-items:center;justify-content:center;gap:10px;background:var(--accent);color:#fff;padding:17px 32px;border:1px solid transparent;border-radius:100px;font:600 15px Manrope,sans-serif;text-decoration:none;">${esc(get('hero.ctaPrimary'))}</a>
          <a class="btn-outline hero-btn ed" data-path="hero.ctaSecondary" href="#services" style="display:inline-flex;align-items:center;justify-content:center;gap:10px;background:transparent;color:var(--accent);padding:17px 32px;border-radius:100px;font:600 15px Manrope,sans-serif;text-decoration:none;border:1px solid var(--accent);">${esc(get('hero.ctaSecondary'))}</a>
        </div>
  ...
  window.GG = {
    async boot(opts){
      ...
      // load content: localStorage draft (admin) > content.json > inlined default.
      // Public visitors hit the CDN-cached file (no cache-buster) for max speed;
      // the editor always pulls the freshest copy.
      let base = null;
      try {
        const r = await fetch(ADMIN ? ('content.json?ts=' + Date.now()) : 'content.json');
